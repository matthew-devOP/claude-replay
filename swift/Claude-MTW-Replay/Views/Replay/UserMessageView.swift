...
struct UserMessageView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 4) {
            Text("USER").font(.caption2).bold().foregroundStyle(appState.theme.accent)
        ...
        }.padding(8).overlay(alignment: .leading) { Rectangle().fill(appState.theme.accent).frame(width: 3) }
    }
}
